...
onMounted(() => {
  sidebar.setPageName('Expenses / Payments')
...
function goToCreate() {
  router.push({ name: 'create_payment' })
}

function goToView(id: number) {
  router.push({ name: 'view_payment', params: { id } })
}

function goToEdit(id: number) {
  router.push({ name: 'edit_payment', params: { id } })
}
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h2 class="text-3xl font-bold tracking-tight">Expenses / Payments</h2>
        <p class="text-muted-foreground">Manage expense transactions and payments.</p>
      </div>
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div v-else-if="transactions.length === 0" class="flex flex-col items-center justify-center py-12 text-center">
          <iconify-icon icon="lucide:arrow-up-circle" class="h-12 w-12 text-muted-foreground mb-4" />
          <h3 class="text-lg font-semibold">No expense transactions found</h3>
          <p class="text-sm text-muted-foreground">Get started by recording a new expense transaction.</p>
