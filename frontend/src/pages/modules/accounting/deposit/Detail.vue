...
const formData = ref({
  ...
  transactableType: 'Student' as 'Student' | 'Payer' | '',
  studentId: '',
  payerId: '',
  paymentMethod: 'cash',
  referenceNumber: '',
...
onMounted(async () => {
  sidebar.setPageName(isEditing.value ? 'Edit Income' : 'Record Income')
  await loadOptions()
  if (isEditing.value) {
...
async function fetchTransaction() {
  try {
    ...
    formData.value = {
      ...
      transactableType: transaction.transactableType || 'Student',
...
async function saveTransaction() {
  ...
  if (!formData.value.transactableType) {
    toast.error('Validation Error', { description: 'Please select Student or Payer' })
    return
  }
  ...
  try {
    ...
    const payload = {
      ...formData.value,
      studentId: formData.value.transactableType === 'Student' ? parseInt(formData.value.studentId) : null,
      payerId: formData.value.transactableType === 'Payer' ? parseInt(formData.value.payerId) : null,
    ...
    if (isEditing.value) {
      ...
      toast.success('Success', { description: 'Income transaction updated successfully' })
    } else {
      await api.createTransaction(payload)
      toast.success('Success', { description: 'Income transaction recorded successfully' })
    }
    router.push({ name: 'deposit' })
  } catch (err: any) {
    const message = err?.response?.data?.detail || `Failed to ${isEditing.value ? 'update' : 'record'} income`
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-muted-foreground">
          {{ isEditing ? 'Update the income transaction details.' : 'Record a new income or deposit transaction.' }}
        </p>
      </div>
    ...
    <form v-else @submit.prevent="saveTransaction" class="space-y-6">
      ...
      <Card>
        ...
        <CardContent class="space-y-4">
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="space-y-2">
              <Label>From <span class="text-red-500">*</span></Label>
      ...
      <Card>
        <CardHeader class="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Add income categories and amounts</CardDescription>
          </div>
      ...
      <div class="flex gap-4">
        <Button type="submit" :disabled="isSaving">
          ...
          {{ isSaving ? 'Saving...' : (isEditing ? 'Update Income' : 'Record Income') }}
        </Button>
